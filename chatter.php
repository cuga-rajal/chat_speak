...
//
// Whats New:
// Version 1.8 improves speech for some long technical phrases such as UUIDs and acronyms.
//
// Instructions:
...
// additional voices if necessary. Edit the $voices section below to match the voices installed on your system.
...
// 
// Once configured, you need to log into a virtual world before running the script. When the
// script is first started, it will check the last-modified date of chatlogs from all avatar
// accounts and will use the chatlog with most recent last-modified date. This generally means the most 
...
// There is no attempt to match male and female voices with avatar genders.
//
...
while(1) {
   ...
   $phrase = preg_replace('/\d{8}-\d{4}-\d{4}-\d{4}-\d{12}/', 'UUID number',$phrase); // Don't speak UUIDs
   ...
   $phrase = str_replace('%',' percent ',$phrase);
   ...
   $phrase = preg_replace('/idk/u',' I dont know ',$phrase);
   $phrase = preg_replace('/ikr/u',' I know, right? ',$phrase);
